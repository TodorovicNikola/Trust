package com.trust40.col_pro_lan.parser.impl;

import com.trust40.col_pro_lan.parser.AbstractParser;
...
public class CBPParser extends AbstractParser {
    ...
    public CollaborativeNetwork parseCollaborativeNetwork(String path) throws ParserConfigurationException, IOException, SAXException, XPathExpressionException {
        CollaborativeNetwork collaborativeNetwork = new CollaborativeNetwork();
//        Document document = prepareDocument(path);
        ...
        for (int i = 0; i < XMLElements.getLength(); i++) {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if(element.getAttribute("xsi:type").equals("mod:CollaborativeNetwork")) {
                    String id = element.getAttribute("id");
                    String name = element.getAttribute("name");
                    String endorsementPolicy = element.getAttribute("endorsementPolicy");

                    collaborativeNetwork = new CollaborativeNetwork(id, name, endorsementPolicy);
                    ...
                    for (int j = 0; j < organizationNodes.getLength(); j++) {
                        ...
                        if (organizationNode.getNodeType() == Node.ELEMENT_NODE) {
                            ...
                            collaborativeNetwork.addOrganization(organization);
                        }
                    }
                }
            }
    ...
    protected Map<String, Object> generateValueMap() {
        ...
        for (String id : relevantElementKeySet) {
            ...
            if (relevantElements.get(id) instanceof ProcessStep) {
                ...
                if(processStep.getContractualObligation() != null) {
