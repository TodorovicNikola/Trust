...
if [ "$CC_SRC_PATH" = "NA" ]; then
	...
	if [ "$CC_NAME" = "cbp-cc" ]; then
		echo $'\e[0;32m'cbp-cc$'\e[0m' chaincode
		CC_SRC_PATH="../cbp-cc"
	elif [ "$CC_NAME" = "ip-cc" ]; then
		echo $'\e[0;32m'ip-cc$'\e[0m' chaincode
		CC_SRC_PATH="../ip-cc"
	else
		echo The chaincode name ${CC_NAME} is not supported by this script
		echo Supported chaincode names are: ip-cc, cbp-cc
		exit 1
	fi
